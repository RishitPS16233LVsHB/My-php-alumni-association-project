<?php 
    ...
    function InsertIntoPostTable($Title,$Description)
    {
        ...
        {
            if(empty($Title))
                error("Post title was empty",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/CreatePost.php");
            if(empty($Description))
                error("Post Description was empty",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/CreatePost.php");
            ...
            if(mysqli_query($connection,$query))
            {
                ...
                if(isset($_FILES['file']) && !empty($_FILES['file']))
                {
                    ...
                    if(!isset($_SESSION['userid']) || !isset($_SESSION['email']) || empty($_SESSION['userid']) || empty($_SESSION['email']))
                        header("Location: ".$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/Login.php");
                        
                    $userID = $_SESSION['userid'];
                    $en_no = $_SESSION['email'];
                    $path = $_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/Uploads/$userID-User-$en_no/".$_FILES['file']['name'];
            ...
            error("not properly looged in",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/Login.php");
    }
    ...
    function InsertIntoEventParticipationTable($EventID)
    {
        ...
        {
            if(empty($EventID))
                error("EventID empty was empty",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/CreatePost.php");
            $userID = $_SESSION['userid'];
            $connection = ConnectToDB();
    ...
    function InsertIntoJobApplyTable($JobID)
    {
        ...
        {
            if(empty($JobID))
                error("JobID empty was empty",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/CreatePost.php");
            $userID = $_SESSION['userid'];
            $connection = ConnectToDB();
    ...
    function InsertIntoEventTable($EventTitle,$EventDescription,$OrganizerName,$EventTime,$Venue,$Contact1,$Contact2,$Email1,$Email2)
    {
        ...
        if(isset($_SESSION['userid']) && !empty($_SESSION['userid']))
        {
            ...
            if(mysqli_query($connection,$query))
            {
                ...
                error("server error please retry",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/profile.php/homePage.php");
            CloseConnection($connection);
        }
        else
            error("not properly looged in",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/Login.php");
    ...
    function InsertIntoUserTechnologyTable($technologyID)
    {
        ...
        if(isset($_SESSION['userid']) && !empty($_SESSION['userid']))
        {
            ...
            {
                CloseConnection($connection);                
                error("server error,please retry",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/settings.php");
            }           
        }
        else
            error("not properly looged in",$_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/Login.php");
    ...
    function DoLogin($Enrollment_Number,$password)
    {
        try
        {
            ...
            {
                //$path = $_SERVER['DOCUMENT_ROOT']."/PHPAlumniProject/Login.php";
